...
class UOM extends App_controller
{
    ...
    public function index()
    {
        ...
        $data['view_path'] = 'pages/categories/list';
        $data['navlink'] = ['mainlink' => 'categories', 'sublink' => ''];
        $data['scripts'] = ['assets/js/pages/categories/list.js', 'assets/js/pages/categories/new.js'];
    ...
    function list()
    {
        ...
        $total_uom = $this->UOM_model->get_uoms('total', $limit, $currentPage, $filters, $search);
        $uoms = $this->UOM_model->get_uoms('list', $limit, $currentPage, $filters, $search);
    ...
    function add()
    {
        ...
        try {
            ...
            $category = $this->UOM_model->get_category_by_name($data['name']);
            if (!empty($category)) {
                $this->sendHTTPResponse(409, [
                    'status' => 'error',
                    'code' => 409,
                    'error' => 'Category is already defined in the system.',
                    'message' => 'Category is already defined in the system.'
                ]);
                return;
            }
            ...
            $createdCategory = $this->UOM_model->add_category($data, $isAuthorized['userid']);
    ...
    public function details($categoryID)
    {
        ...
        $category = $this->UOM_model->get_category_by_id($categoryID);
    ...
    function update($id)
    {
        ...
        try {
            ...
            // Check data is already created with given filter
            $category = $this->UOM_model->get_category_by_id($id);
            if (empty($category)) {
            ...
            // Save Data to the product table
            $updatedCategory = $this->UOM_model->update_category($id, $data, $isAuthorized['userid']);
    ...
    function delete($id)
    {
        ...
        $category = $this->UOM_model->get_category_by_id($id);
        $result = $this->UOM_model->delete_category_by_id($id);
        if ($result) {
            $this->output
                ->set_content_type('application/json')
